/**
 * Unit tests for the KEY-SELECTION half of the public token route — the logic in
 * GatewayRouter.resolvePublicTokenKey(token): base64url-decode the token's UNTRUSTED
 ...
 * The chosen key is then handed to verifyPublicToken, whose HMAC is what makes `a`
 ...
 *   resolvePublicTokenKey is a private method on GatewayRouter, and importing
 ...
 *   drive the REAL crypto (sign/verify imported from src/api/public-token.ts). This
 ...
 *
 * The replica below is a byte-for-byte copy of resolvePublicTokenKey's body
 * (gateway-router.ts) — keep it in sync if that method changes.
 */
import { signPublicToken, verifyPublicToken } from '../../src/api/public-token';

// Mirror of the ApiKey fields the predicate reads (src/types.ts).
type TestApiKey = { key: string; agents?: string[] | '*'; admin?: boolean };

// ── exact replica of GatewayRouter.resolvePublicTokenKey ─────────────────────
function resolvePublicTokenKey(token: string, keys: TestApiKey[]): string {
...
function resolveAndVerify(token: string, keys: TestApiKey[]) {
  return verifyPublicToken(token, resolvePublicTokenKey(token, keys));
}
...
const sign = (a: string, secret: string) =>
  signPublicToken({ k: 'media', a, p: 'media/c/f.png', e: Date.now() + 60_000 }, secret);

describe('resolvePublicTokenKey() — key selection precedence', () => {
  test('agent-scoped key is chosen when a matches its agents list', () => {
    const keys = [AGENT_KEY, WILDCARD_KEY, ADMIN_KEY];
    expect(resolvePublicTokenKey(sign('agentA', AGENT_KEY.key), keys)).toBe(AGENT_KEY.key);
  });

  test('wildcard "*" key is chosen for an agent no scoped key names', () => {
    const keys = [AGENT_KEY, WILDCARD_KEY];
    expect(resolvePublicTokenKey(sign('agentZ', WILDCARD_KEY.key), keys)).toBe(WILDCARD_KEY.key);
  });

  test('admin key is chosen when neither a scoped nor a wildcard key matches', () => {
    const keys = [AGENT_KEY, ADMIN_KEY];
    expect(resolvePublicTokenKey(sign('agentZ', ADMIN_KEY.key), keys)).toBe(ADMIN_KEY.key);
  });

  test('no matching key (only other agents scoped) → "" ', () => {
    const keys: TestApiKey[] = [{ key: 'key-other', agents: ['agentB'] }];
    expect(resolvePublicTokenKey(sign('agentA', 'whatever'), keys)).toBe('');
  });

  test('empty config → "" ', () => {
    expect(resolvePublicTokenKey(sign('agentA', 'whatever'), [])).toBe('');
  });

  test('malformed token (no ".") → "" (never touches the key list)', () => {
    expect(resolvePublicTokenKey('garbage', [WILDCARD_KEY])).toBe('');
  });

  test('token body without a string `a` → "" ', () => {
    const body = Buffer.from(JSON.stringify({ k: 'media', p: 'x', e: 1 }), 'utf-8')
      .toString('base64')
      .replace(/\+/g, '-')
      .replace(/\//g, '_')
      .replace(/=+$/, '');
    expect(resolvePublicTokenKey(`${body}.AAAA`, [WILDCARD_KEY])).toBe('');
  });
});
...
describe('resolve ↔ verify — the route contract (403 vs serve)', () => {
  ...
  test('WRONG signing key (signed with a key that is not the one a resolves to) → null (403)', () => {
    ...
    expect(resolvePublicTokenKey(token, keys)).toBe(AGENT_KEY.key); // a selected the scoped key
  ...
  test('security property: sign(K1) but claim an a that resolves to K2≠K1 → verify fails', () => {
    ...
    // resolvePublicTokenKey's .find hits K2 first (it matches agentA), verify(K2) fails.
    const K1 = WILDCARD_KEY;
    const K2 = AGENT_KEY;
    const keys = [K2, K1]; // .find scans in array order; K2 matches agentA first
    const forged = signPublicToken({ k: 'media', a: 'agentA', p: 'media/c/f.png', e: Date.now() + 60_000 }, K1.key);
    expect(resolvePublicTokenKey(forged, keys)).toBe(K2.key);
  ...
  test('determinism: the same agent selects the same key across independent tokens', () => {
    const keys = [AGENT_KEY, WILDCARD_KEY, ADMIN_KEY];
    const k1 = resolvePublicTokenKey(sign('agentA', AGENT_KEY.key), keys);
    const k2 = resolvePublicTokenKey(sign('agentA', AGENT_KEY.key), keys);
    expect(k1).toBe(k2);

...
export class SessionStore {
   ...
   */
  private resolveTelegramDir(agentId: string, chatId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): string {
   ...
   */
  private resolveTelegramIndexPath(agentId: string, chatId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): string {
   ...
   */
  private resolveTelegramSessionPath(agentId: string, chatId: string, sessionId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): string {
   ...
   */
  async loadIndex(agentId: string, chatId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<SessionIndex | null> {
   ...
   */
  async saveIndex(agentId: string, chatId: string, index: SessionIndex, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<void> {
    const indexPath = this.resolveTelegramIndexPath(agentId, chatId, channel);
   ...
   */
  private async loadOrCreateIndexUnlocked(agentId: string, chatId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<SessionIndex> {
  ...
  async getOrCreateIndex(agentId: string, chatId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<SessionIndex> {
   ...
   */
  async createTelegramSession(agentId: string, chatId: string, name?: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<SessionMeta> {
    const queue = this.getTelegramQueue(agentId, chatId);
  ...
  async listSessions(agentId: string, chatId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<SessionIndex> {
    return this.getOrCreateIndex(agentId, chatId, channel);
  }

  async getActiveSessionId(agentId: string, chatId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<string> {
    const index = await this.getOrCreateIndex(agentId, chatId, channel);
    return index.activeSessionId;
  }

  async setActiveSession(agentId: string, chatId: string, sessionId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<void> {
    const queue = this.getTelegramQueue(agentId, chatId);
    await queue.add(async () => {
  ...
  async deleteTelegramSession(agentId: string, chatId: string, sessionId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<void> {
    const queue = this.getTelegramQueue(agentId, chatId);
    await queue.add(async () => {
      const index = await this.loadIndex(agentId, chatId, channel);
      if (!index) {
  ...
  async clearTelegramSessionHistory(agentId: string, chatId: string, sessionId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<void> {
  ...
  async updateSessionMeta(
    ...
    channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram',
  ): Promise<void> {
    const queue = this.getTelegramQueue(agentId, chatId);
    await queue.add(async () => {
      const index = await this.loadIndex(agentId, chatId, channel);
  ...
  async loadTelegramSession(agentId: string, chatId: string, sessionId: string, channel: 'telegram' | 'discord' | 'api' | 'line' | 'slack' = 'telegram'): Promise<Message[]> {
  ...
  async getAllSessionMeta(agentId: string): Promise<Map<string, { name: string; imageConfig?: ImageParams; model?: string }>> {
    ...
    const reads = entries
      .filter((e) => e.isDirectory() && (e.name.startsWith('telegram-') || e.name.startsWith('discord-') || e.name.startsWith('line-') || e.name.startsWith('slack-') || e.name.startsWith('api-')))
      .map(async (e) => {
        const channel = e.name.startsWith('discord-')
          ? 'discord'
          : e.name.startsWith('line-')
            ? 'line'
